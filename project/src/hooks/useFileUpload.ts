...
const N8N_WEBHOOK_URL = 'https://njord-gear.app.n8n.cloud/webhook-test/d2855857-3e7b-4465-b627-89ed188f2151';
...
export const useFileUpload = () => {
        ...
        reader.onload = async (e) => {
          ...
          img.onload = async () => {
            try {
              ...
              if (currentProject) {
                thumbnailPath += `/projects/${currentProject.id}`;
                if (currentFolder) {
                  thumbnailPath += `/folders/${currentFolder.id}`;
                }
              }
  ...
  const uploadFiles = async (files: FileList | File[], manualTags: string[] = [], autoTaggingEnabled: boolean = true) => {
    ...
    try {
      ...
      if (currentProject) {
        console.log('Uploading to project:', currentProject.name);
        if (currentFolder) {
          console.log('Uploading to folder:', currentFolder.name);
        } else {
      ...
      const uploadPromises = fileArray.map(async (file, index) => {
        ...
        try {
          ...
          if (currentProject) {
            filePath += `/projects/${currentProject.id}`;
            if (currentFolder) {
              filePath += `/folders/${currentFolder.id}`;
            }
          }
          filePath += `/${fileName}`;
