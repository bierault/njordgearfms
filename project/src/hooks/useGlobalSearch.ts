...
export const useGlobalSearch = (query: string, selectedTags: string[] = []): GlobalSearchResult => {
  ...
  const searchFiles = useCallback(async (searchQuery: string, tags: string[]) => {
    ...
    try {
      ...
      let searchQueryBuilder = supabase
        ...
        .limit(150); // Increased limit for better results
      ...
      // Apply tag filters if tags are selected
      if (tags.length > 0) {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      searchFiles(query, selectedTags);
    }, 200); // Reduced debounce for faster response

    return () => clearTimeout(timeoutId);
  }, [query, selectedTags, searchFiles]);
